...
void servo_control(int angle);
{
	...
	if(angle<-60)
	angle=-60;
	servoduty=int(1500+servoduty*500/90);
...
{
	int speednum;
	speednum=speedduty*实测脉冲比值;//脉冲比值需测试
...
{
	FTM_PWM_Duty（FTM0，CH1，duty1);//野火驱动FTM0，CH3，CH4
	FTM_PWM_Duty（FTM0, CH2, duty2);//电机与野火驱动红黑线暗示错误，实际红线正极时反转，但不影响编程
} 